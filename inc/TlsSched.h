...
/*_____________________________________________________________________________
 |
 | TlsSched.h offers facilities to manage multiple objects 'actors' that act
 | independently.
...
/*_____________________________________________________________________________
 |
 | Dates:
 | Dates are expressed as an integer in an arbitrary unit.
 | You should always expect dates to wrap around the integer range.
 | Therefore, you should never expect that a date t0 preceding a date t1 will
 | result in (t0<t1) to be true.
 | Instead: you should write your application so that the lag between two dates
 | never exceed the int max value. You can then test if one dates precede the
 | other with the test: ((t0-t1)<0)
 |_____________________________________________________________________________
*/
...
/*_____________________________________________________________________________
 |
 | Actors:
 |    Actors are given a limit date (in arbitrary subjective unit) up to which 
 | they are allowed to act. They mustn't go past the deadline,  but can
 | do less.
 |    When returning, they must give the date at which they stopped.
 |_____________________________________________________________________________
*/

typedef struct { 
    struct TlsActor *Static;
    char *Id;
} TlsActor;
struct TlsActor { 
    /* Act for up to tick numbers.  Returns the tick numbers left */
    int (*Perform)(TlsActor *this,int MaxEndDate);
};
extern TlsActor TlsActorNull;
...
typedef struct {
    /* The manager is bound to be managed by a higher up entity, possibly the root thread.*/
    TlsActor TlsActor; 
    struct TlsStage *Static; 
} TlsStage;

typedef struct {struct TlsStageName *Static; } TlsStageName;
struct TlsStageName {
    TlsStage *(*GetStage)(TlsStageName *this);
    void (*AlterPriority)(TlsStageName *this,TlsStageName *ref,int offset);
    int (*GetDate)(TlsStageName *this,int MyDate); 
    void (*Sleep)(TlsStageName *this,int date,int delay); 
    void (*Hang)(TlsStageName *this);
    void (*End)(TlsStageName *this);
    void (*Awaken)(TlsStageName *this,int date); 
    void (*FlushEvents)(TlsStageName *this);
};
...
typedef struct {struct TlsSchMsg *Static;} TlsSchMsg;
struct TlsSchMsg {
    void (*Interpret)(TlsSchMsg *this,TlsActor *Tgt);
    void (*Abort)(TlsSchMsg *this);
};
typedef struct {struct TlsSchEvent *Static; } TlsSchEvent;
struct TlsSchEvent {
    void (*Remove)(TlsSchEvent *this);
};

struct TlsStage {
    TlsStageName *(*EnterActor)(TlsStage *this,TlsActor *Actor,int date);
    TlsSchEvent *(*InsertEvent)(TlsStage *this,int Date,TlsSchMsg *Msg,TlsStageName *Tgt); // date is relative to the deadline
};
extern TlsStage TlsStageNull;
...
/*_____________________________________________________________________________
 ...
 |
 | The fiber actors main task receive their TlsStageName as a parameter. They must
 | use the 'GetDate(0)' method to get the next allocated timeslice.
 | Each time they use the 'GetDate(...)' method, they hand the program flow to
 | the stage. The parameter given to the GetDate() method is the date at which
 | they decided to hand the program flow.
 |
 ...
 | period should use 'Sleep()' instead of 'GetTicks()'. 'Sleep()' move the actor
 ...
 | actor, they should awaken him first before using 'GetTicks()'.
...
typedef struct { struct TlsSchThread *Static; } TlsSchThread;
struct TlsSchThread { 
    void (*Main)(TlsSchThread *this,TlsStageName *Name); 
};

void TlsSchFiberLaunch(
    TlsStage *Stage,int date,char *Label,TlsSchThread *Main,
...
extern TlsSchMsg TlsSchNullMsg; 
/* The target of this message will be called with a 0 timesliced allocated.
   ...
   in time...  
*/
extern TlsSchMsg TlsSchMsgGoFirst; 
/* Useful message to access specific pieces of data */
TlsSchMsg *TlsSchMsgCharPtr(TlsStageName *t,void (*Interpret)(char *dst),char *dst);
TlsSchMsg *TlsSchMsgChar(TlsStageName *t,void (*Interpret)(char value),char Value);

TlsSchMsg *TlsSchMsgIntPtr(TlsStageName *t,void (*Interpret)(int *dst),int *dst);
TlsSchMsg *TlsSchMsgInt(TlsStageName *t,void (*Interpret)(int value),int Value);

TlsSchMsg *TlsSchMsgPtrPtr(TlsStageName *t,void (*Interpret)(void **dst),void **dst);
TlsSchMsg *TlsSchMsgPtr(TlsStageName *t,void (*Interpret)(void *Value),void *Value);

TlsSchMsg *TlsSchMsgIdxCharPtr(TlsStageName *t,void (*Interpret)(int Idx,char *dst),int Idx,char *dst);
TlsSchMsg *TlsSchMsgIdxChar(TlsStageName *t,void (*Interpret)(int Idx,char value),int Idx,char value);

TlsSchMsg *TlsSchMsgIdxIntPtr(TlsStageName *t,void (*Interpret)(int Idx,int *dst),int Idx,int *dst);
TlsSchMsg *TlsSchMsgIdxInt(TlsStageName *t,void (*Interpret)(int Idx,int Value),int Idx,int Value);

TlsSchMsg *TlsSchMsgIdxPtrPtr(TlsStageName *t,void (*Interpret)(int Idx,void **dst),int Idx,void **dst);
TlsSchMsg *TlsSchMsgIdxPtr(TlsStageName *t,void (*Interpret)(int Idx,void *Value),int Idx,void *Value);

TlsSchMsg *TlsSchMsgSD(TlsStageName *t,void (*Interpret)(void *dst,void *src),void *dst,void *src);
TlsSchMsg *TlsSchMsgSDS(TlsStageName *t,void (*Interpret)(void *dst,void *src,int siz),void *dst,void *src,int siz);
